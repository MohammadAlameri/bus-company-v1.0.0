...
function loadAppointments() {
    ...
    if (appointmentFilter) {
        appointmentFilter.addEventListener('change', () => {
            ...
            const searchTerm = appointmentSearch ? appointmentSearch.value.toLowerCase() : '';
...
async function fetchAppointments() {
    try {
        ...
        showMessage('Error loading appointments data', 'error');
    }
}
...
function addAppointmentToTable(appointment) {
    ...
    // Get passenger info
    const passengerName = appointment.passengerDetails ? appointment.passengerDetails.name : 'Unknown Passenger';
    ...
    if (appointment.paymentDetails) {
        paymentStatus = appointment.paymentDetails.paymentStatus === 'completed' ? 
...
function addAppointmentActionListeners() {
    // View buttons
    const viewButtons = document.querySelectorAll('#appointments-table .view-btn');
    ...
    // Approve buttons
    const approveButtons = document.querySelectorAll('#appointments-table .approve-btn');
    ...
    // Reject buttons
    const rejectButtons = document.querySelectorAll('#appointments-table .reject-btn');
...
async function showAppointmentDetails(appointmentId) {
    try {
        ...
        const passengerDetails = `
            ...
            <p><strong>Nationality:</strong> ${passenger.nationalityCountry || 'N/A'}</p>
        `;
        
        // Format trip details
        const tripDate = trip.date ? 
            new Date(trip.date.toDate ? trip.date.toDate() : trip.date).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
            }) : 'N/A';
        ...
        const tripDetails = `
            ...
            <p><strong>Seat Number:</strong> ${appointment.seatNumber || 'N/A'}</p>
        `;
        ...
        const paymentDetails = `
            ...
            <p><strong>Amount:</strong> ${payment.amount || 'N/A'} ${payment.currency || ''}</p>
            <p><strong>Method:</strong> ${payment.paymentMethod || 'N/A'}</p>
            <p><strong>Transaction ID:</strong> ${payment.transactionID || 'N/A'}</p>
            <p><strong>Date:</strong> ${payment.createdAt ? 
                new Date(payment.createdAt.toDate ? payment.createdAt.toDate() : payment.createdAt)
